...
type CycleVente = {
  ...
  nom: string;
  unite: string;
  /** Récolté moins déjà vendu, tel que calculé par vue_rentabilite_cycles. */
...
export default function EcranVente() {
  ...
  const [cycles, setCycles] = useState<CycleVente[]>([]);
  const [chargement, setChargement] = useState(true);
  const [envoi, setEnvoi] = useState(false);
  const [erreur, setErreur] = useState<string | null>(null);

  // ---------------------------------------------------------------------------
  // Trois requêtes en parallèle plutôt qu'une jointure imbriquée : l'unité vit
  // dans speculations, le reliquat dans vue_rentabilite_cycles. La table des
  // spéculations tient en une dizaine de lignes, la rapatrier coûte moins cher
  // qu'une requête fragile.
  // ---------------------------------------------------------------------------
  useEffect(() => {
    let actif = true;
    ...
    (async () => {
      const [resCycles, resSpec, resRenta] = await Promise.all([
        ...
        supabase
          ...
          .select("cycle_id, quantite_restante")
          .eq("statut", "actif"),
      ]);
      ...
      const restants = new Map<string, number | null>(
        (
          (resRenta.data ?? []) as { cycle_id: string; quantite_restante: number | null }[]
        ).map((r) => [r.cycle_id, r.quantite_restante]),
      );

      const liste: CycleVente[] = (
        (resCycles.data ?? []) as { id: string; nom: string; speculation_id: string | null }[]
      ).map((c) => ({
        id: c.id,
        nom: c.nom,
        unite: (c.speculation_id ? unites.get(c.speculation_id) : null) ?? "unité",
        quantiteRestante: restants.get(c.id) ?? null,
      }));
